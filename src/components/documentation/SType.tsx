  ...
  return (
    <div>
      ...
      {sType?.kind === 'INPUT_OBJECT' && (
        <h3 className="font-bold my-2">{`${texts.main.docs.inputFields} ${sType.name}`}</h3>
      )}
      {sType?.kind === 'SCALAR' && (
        <h3 className="my-2">{`${sType.name} ${texts.main.docs.kind} 'Scalar'`}</h3>
      )}
      {sType?.kind === 'ENUM' && (
        <h3 className="my-2">{`${sType.name} ${texts.main.docs.kind} 'Enum'`}</h3>
      )}
      {sType && sType.fields ? (
